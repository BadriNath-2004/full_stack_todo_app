...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.message || t("login_failed"));
  ...
  return (
    <Layout>
      <div className="max-w-md w-full mx-auto mt-20 bg-white shadow-xl rounded-xl p-8 space-y-6 border border-gray-100">
        <h2 className="text-3xl font-bold text-gray-800 text-center">{t("login")}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            className="w-full bg-primary text-white py-2 rounded-lg font-medium hover:bg-primary-dark transition duration-300"
          >
            {t("login")}
          </button>
        </form>
